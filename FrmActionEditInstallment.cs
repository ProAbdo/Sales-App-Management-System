...
namespace SalesApp
{
    public partial class FrmActionEditInstallment : Form
    {
        string customer_number, item_number;
        public FrmActionEditInstallment()
        {
            InitializeComponent();
        }
        public FrmActionEditInstallment(string customer_number,string item_number)
        {
        ...
        {
            if (dgvEditInstallment.Columns[e.ColumnIndex].Name == "اضافة")
            {
                if (dgvEditInstallment.Rows[e.RowIndex].Cells["الحالة"].Value.ToString() == "تم الدفع")
                {
                    string installment_num = dgvEditInstallment.Rows[e.RowIndex].Cells["رقم_القسط"].Value.ToString();
                    new FrmMain().OpenForm(new FrmAddEditInstallment(customer_number, item_number, installment_num));
                }
                else
                {
                    MessageBox.Show("!!! لم يتم دفع القسط", "خطأ", MessageBoxButtons.OK, MessageBoxIcon.Error);

                }
            }
        }

        private void FrmActionEditInstallment_Load(object sender, EventArgs e)
        {
            reload();
        }
    }
